...
import styles from "../../styles/Sidebar.module.css"; // Adjust this path based on your file structure
...
const NavItem = ({ icon, label, hasDropdown, path }) => {
  const router = useRouter();

  const handleClick = () => {
    router.push(path); // Navigate to the corresponding path when clicked
  };
  ...
  return (
    ...
    <div className={styles.navItemContent}>
      <img src={icon} alt="" className={styles.navItemIcon} />
      <div className={styles.navItemLabel}>{label}</div>
    </div>
    {hasDropdown && (
      <img
      loading="lazy"
